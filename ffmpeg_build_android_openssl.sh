...
function build_one()
{
	...
	export PKG_CONFIG_PATH=/Users/wanghong/Downloads/freetype-2.6.3/out/$_abi/lib/pkgconfig:/Users/wanghong/Developments/libwebp-ndk/obj/local/$_abi:/Users/wanghong/Developments/openssl-1.0.2k/build_out/android/armeabi/lib/pkgconfig:/Users/wanghong/Developments/rtmpdump-android/jni/out/android/armeabi/lib/pkgconfig:/Users/wanghong/Developments/rubberband/out/lib/pkgconfig
	...
	./configure --enable-cross-compile \
		...
		--sysroot=$_sysroot \
		--disable-encoders \
		--enable-encoder=aac,libx264,mjpeg,png,libwebp,dnxhd \
		--disable-decoders \
		--enable-decoder=aac,mjpeg,png,webp,h264,h264_mediacodec,dnxhd \
		--disable-filters \
		--enable-filter=scale,scale2ref,copy,movie,resample,transpose,overlay,drawtext,rubberband \
		--disable-demuxers \
		--enable-demuxer=aac,h264,mov,image2,concat \
		--disable-muxers \
		--enable-muxer=h264,mp4,mov,webp,flv \
		--enable-openssl \
		--enable-jni \
		--enable-mediacodec \
		--enable-librtmp \
		...
		--extra-ldflags="-L../x264 -L/Users/wanghong/Developments/android-ndk-r12b/sources/cxx-stl/gnu-libstdc++/4.9/libs/armeabi-v7a -lgnustl_static" \
		...
		if [ $_host == "aarch64-linux" ];then
			...
			sed -i ".bak" '/EXTRALIBS/{s#-L/opt/local/lib ##;}' config.mak
		fi
		...
		ANDROID_JNI_LIBS=/Users/wanghong/AndroidStudioProjects/Amegracam/livecamera/src/main/jniLibs/$_abi
		...
		${_cross_prefix}gcc -shared -o out/$_abi/libffmpeg.so -Wl,-soname,libffmpeg.so -Wl,--whole-archive `find ../x264 out/$_abi -name "*.a" | xargs` -Wl,--no-whole-archive \
			--sysroot $_sysroot `pkg-config --libs libwebp freetype2 librtmp openssl rubberband` -lm -lz
